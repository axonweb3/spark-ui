import { Paginated, StakeRecord } from '@/server/api/type';
import { Atom, useAtomValue } from 'jotai';
import { loadable } from 'jotai/utils';
import { AtomFamily } from 'jotai/vanilla/utils/atomFamily';
import { useEffect, useMemo, useState } from 'react';

export function usePaginatedAtomQuery(asyncAtom: AtomFamily<any, Atom<Promise<Paginated<any>>>>, address?: string) {
  const [pageNumber, setPageNumber] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [total, setTotal] = useState(0);
  const [data, setData] = useState<StakeRecord[]>([]);
  const queryParams = useMemo(() => ({ address, pageNumber, pageSize }), [address, pageNumber, pageSize]);
